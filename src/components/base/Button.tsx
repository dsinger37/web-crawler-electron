...
export const Button = forwardRef(function Button(
  ...
  // NOTE: Test this to make sure it behaves the same as the above
  function getClasses() {
    // Always include className and styles.base
    const classList = [className, styles.base];

    if (outline) {
      // If outline is true, add styles.outline
      classList.push(styles.outline);
    } else if (plain) {
      // If plain is true, add styles.plain
      classList.push(styles.plain);
    } else {
      // Default case, add styles.solid and the color class
      const colorClass = styles.colors[color ?? "dark/zinc"];
      classList.push(styles.solid, colorClass);
    }

    // Join the classes using clsx for potential space-separated class names
...
export const TouchTarget = ({ children }: { children: ReactNode }) => {
  return (
    <>
      {children}
      <span
